...
<form class="pkp_form" id="swordServerSettingsForm" method="post" action="{url router=$smarty.const.ROUTE_COMPONENT op="manage" category="gateways" plugin="swordServer" verb="settings" save=true}">
	<div id="swordServerSettings">
		...
		{fbvFormArea id="swordServerSettingsFormArea"}
			{capture assign="exampleDepositPointUrl"}{url router=$smarty.const.ROUTE_PAGE page="gateway" path="gateway"|to_array:"plugin":"swordServer":"sections":"1"}{/capture}
			{capture assign="serviceDocumentUrl"}{url router=$smarty.const.ROUTE_PAGE page="gateway" path="gateway"|to_array:"plugin":"swordServer":"servicedocument"}{/capture}
